    ...
    <div class="page">
        ...
        <ul>
            <li class="li1" v-for="(list,index) in newList" :class="{'eidting':curIndex===index}">
                ...
                <input type="text" class="text2" v-model="list.name" @keyup.esc="cancelEdit(list)" @blur="edited" @focus="editBefore(list.name)" @keyup.13="edited" v-focus>
            </li>
        </ul>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        edited(){
            ...
            //还有一个要注意的就是虽然li遍历的是newList，比如改了newList的这一项（{name:"HTML5",status:false}），比如改成这样（{name:"HTML",status:true}）。实际上prolist的这一项（{name:"HTML5",status:false}），
            //也会被改成（{name:"HTML",status:true}）。因为这里是一个对象，而且公用一个堆栈！修改其中一个，另一个会被影响到
            this.curIndex="";
        },
        //取消修改
        cancelEdit(val){
...
<style>
.page{font-family: "微软雅黑";font-size: 0.3rem;line-height: 0.5rem;}
